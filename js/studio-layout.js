/**
 * スタジオ用の配置編集。
 * 地図・カード位置・カード倍率の親は「地域×解像度」1つ。
 * 北海道の今日/明日×天気/降水はその子、東北も同様。地方同士では共有しない。
 */

import { canonicalContent, canonicalRegion } from "./catalog.js?v=pref387";
...
function attachSharedMap(regionState = {}, entry = {}) {
  const next = layoutEntryFrom(entry);
  return {
    ...regionState,
    sharedMap: {
      map: next.map,
  ...
  try {
    ...
    for (const [regionId, regionDef] of Object.entries(shippedDefaults.layouts || {})) {
      ...
      for (const [vpKey, entry] of Object.entries(regionDef.viewports || {})) {
        const local = viewports[vpKey];
        const localHasCards = local && Object.keys(local.cards || {}).length > 0;
        const localHasMap = local && local.map
          && (Math.abs(Number(local.map.x) || 0) > 0.01
    ...
    for (const [regionId, prev] of Object.entries(all)) {
      ...
      all[id] = attachSharedMap({ ...(all[id] || prev), viewports: { ...(all[id]?.viewports || prev.viewports || {}) } }, fallback);
      changed = true;
    }
...
export function saveLayout(regionId, layout, contentId = "today_weather", width = 0, height = 0, options = {}) {
  ...
  const prevSlice = (vpKey && prev.viewports?.[vpKey])
    ...
    || prev
    || {};
  const sharedPrev = prev.sharedMap || {};
  const prevCards = { ...(sharedPrev.cards || prevSlice.cards || {}) };
  const incoming = layout.cards || {};
  const nextCards = options.mapOnly
    ? prevCards
    : (Object.keys(incoming).length ? { ...prevCards, ...incoming } : prevCards);
  const entry = {
    map: layout.map || sharedPrev.map || prevSlice.map || emptyLayout().map,
    okinawa: layout.okinawa || sharedPrev.okinawa || prevSlice.okinawa || emptyLayout().okinawa,
    precipLegend: layout.precipLegend || sharedPrev.precipLegend || prevSlice.precipLegend || emptyLayout().precipLegend,
    cards: { ...nextCards },
    cardsPop: { ...nextCards },
    rev: Math.max(Date.now(), Number(shippedDefaults.rev) || 0, Number(prevSlice.rev) || 0) + 1
  };
  const viewports = { ...(prev.viewports || {}) };
...
/** カード倍率の親キー。全国は national、地方は地域ID（hokkaido / tohoku …）。 */
function cardScaleScope(regionId = "national") {
  return canonicalRegion(regionId);
}

function cardScaleFallbackScopes(regionId = "national") {
  const scope = cardScaleScope(regionId);
  return scope === "national" ? ["national"] : [scope, "regional"];
}
...
function cardScaleLookupKeys(contentId = "today_weather", regionId = "national", width = 0, height = 0) {
  const scopes = cardScaleFallbackScopes(regionId);
  const w = Math.round(Number(width) || 0);
  const h = Math.round(Number(height) || 0);
  const sized = w > 0 && h > 0;
  ...
  for (const variant of variants) keys.push(variant);
  return [...new Set(keys)];
}
  ...
  try {
    ...
    if (!Number.isFinite(value)) {
      const variants = isSharedMapContent(contentId)
        ? ["map", "weather", "pop"]
        : [cardScaleVariant(contentId), "weather", "pop"];
      for (const stores of [[all], [shippedDefaults.cardScales]]) {
        for (const scope of cardScaleFallbackScopes(regionId)) {
...
function titleScaleGroup(contentId = "today_weather") {
  const id = canonicalContent(contentId);
  return isSharedMapContent(id) ? "map" : id;
}

function titleScaleStoreKey(width = 0, height = 0, regionId = "national", contentId = "today_weather") {
  const w = Math.round(Number(width) || 0);
  const h = Math.round(Number(height) || 0);
  const region = canonicalRegion(regionId);
  const vpKey = w > 0 && h > 0 ? viewportSizeKey(w, h) : "default";
  return `${region}:${titleScaleGroup(contentId)}:${vpKey}`;
}
...
export function saveTitleScale(scale, width = 0, height = 0, regionId = "national", contentId = "today_weather") {
  ...
  const key = titleScaleStoreKey(width, height, regionId, contentId);
...
export function snapshotLayoutDefaults(regionId, layout, contentId, width, height, cardScale, titleScale) {
  ...
  const titleKey = titleScaleStoreKey(w, h, regionId, contentId);
  return {
    rev: entry.rev,
    layouts: {
      [regionId]: {
...
export function bindCardEditor(cardsEl, layout, regionId, contentId = "today_weather", onChange, screen) {
  ...
  const currentScale = () => {
    const live = Number.parseFloat(screen?.style?.getPropertyValue("--card-scale"));
